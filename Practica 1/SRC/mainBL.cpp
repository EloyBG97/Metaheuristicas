...
int main(int argc, char **argv) {
  ...
  for(i = 0; i < N_PARTICIONES; ++i) {
    ...
    solucion[i] = BusquedaLocalAprendizaje(training, seed);
    tdespues = high_resolution_clock::now();

    tiempo_transcurrido[i]  = duration_cast<duration<double> >(tdespues - tantes);

    for(unsigned int j = 0; j < solucion[i].first.size(); ++j)
      if(solucion[i].first[j] < 0.2) {
        solucion[i].first[j] = 0;
      }

    solucion[i].second.first = validacionTest(solucion[i].first, training, test);

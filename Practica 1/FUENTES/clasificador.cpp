...
pair<float, float> fObjetivo(const vector<individuo::value_type>&W, const muestra &training, const muestra &test) {
  return pair<float, float>(tasaClas(W, training, test),tasaRed(W, 0.2));
}

float tasaClas(const vector<individuo::value_type>&W, const muestra &training, const muestra &test) {
  typename muestra::size_type bien_training, bien_test;

  bien_training = validacionTraining(W, training);

  bien_test = validacionTest(W, training, test);

  return ((bien_test + bien_training) / (float) (training.getSize() + test.getSize())) * 100;

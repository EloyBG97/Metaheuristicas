...
int main(int argc, char **argv) {
  ...
  vector<typename individuo::value_type> W(test.getIndividuo(0).getSize(), 1);
vector<float> tasa_training(N_PARTICIONES);
  ...
  for(i = 0; i < N_PARTICIONES; ++i) {
    ...
    tasa_training[i] = solucion[i].first;
  ...
  for( i = 0; i < N_PARTICIONES; ++i) {
    ...
    cout << "\tTasa Clasificacion Test: " << solucion[i].first << endl;
    cout << "\tTasa Reduccion: " << solucion[i].second << endl;
